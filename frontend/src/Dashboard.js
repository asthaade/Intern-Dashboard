    ...
    <div className="animate-fade-in">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {rewards.map(reward => {
            const isUnlocked = internData.donations >= reward.threshold;
            return (
              <div key={reward.title} className={`p-6 rounded-xl transition-all transform ${isUnlocked ? 'bg-white shadow-lg hover:-translate-y-1' : 'bg-gray-50 shadow-inner'}`}>
                <div className={`transition-opacity ${!isUnlocked && 'opacity-40'}`}>
                  {reward.icon}
                  <h4 className="font-bold text-lg text-center">{reward.title}</h4>
                  <p className="text-sm text-gray-600 text-center mb-4">{reward.description}</p>
                  <div className="w-full bg-gray-200 rounded-full h-2.5">
                    <div className="bg-indigo-600 h-2.5 rounded-full" style={{ width: `${Math.min((internData.donations / reward.threshold) * 100, 100)}%` }}></div>
                  </div>
                  <p className="text-xs text-center mt-2 font-medium text-gray-500">
                    ${internData.donations.toLocaleString()} / ${reward.threshold.toLocaleString()}
                  </p>
                </div>
              </div>
            );
